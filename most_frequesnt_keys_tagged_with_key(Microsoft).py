...
limit, query = 15000, '@Microsoft'
cursor = tweepy.Cursor(api.search, q=query, since="2017-03-01", until="2017-04-01", lang='en').items(limit)
tweets = get_tweets('tweets_in_March_by_keyword_Microsoft.json', load_existing=True, cursor_obj=cursor)  # load tweets locally
...
plt.title('Statistics about most frequent tags together with tag "Microsoft" (during March)')
